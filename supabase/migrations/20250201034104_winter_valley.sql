...
BEGIN
    ...
    DROP POLICY IF EXISTS "Rentals viewable by client or admin" ON rentals;
    DROP POLICY IF EXISTS "Clients create rentals" ON rentals;
    DROP POLICY IF EXISTS "Admin updates rentals" ON rentals;
END $$;
...
CREATE POLICY "Quote access for clients and admin"
  ON quotes
  FOR SELECT
  TO authenticated
  USING (
...
CREATE POLICY "Quote creation for clients"
  ON quotes
  FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = client_id);

CREATE POLICY "Quote updates for admin"
  ON quotes
  FOR UPDATE
  TO authenticated
  USING (is_admin())
...
CREATE POLICY "Rental access for clients and admin"
  ON rentals
  FOR SELECT
  TO authenticated
  USING (
...
CREATE POLICY "Rental creation for clients"
  ON rentals
  FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = client_id);

CREATE POLICY "Rental updates for admin"
  ON rentals
  FOR UPDATE
  TO authenticated
  USING (is_admin())
