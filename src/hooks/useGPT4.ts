...
export function useGPT4() {
  const [isProcessing, setIsProcessing] = useState(false);

  const searchWithGPT4 = async (query: string): Promise<GPTResponse | null> => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Falha ao processar com GPT-4');
      }
      ...
      try {
        ...
        console.error('Erro ao analisar resposta do GPT:', parseError);
      ...
      console.error('Erro ao processar com GPT-4:', error);
      return null;
    } finally {
